...
start(GroupName) ->
  register(GroupName, spawn(fun() -> start1() end)).
  ...
  receive
    {join, Pid, Who} ->
      case lists:keysearch(Who, 1, L) of
        false ->
          L1 = L ++ [{Who, Pid}],
          {registered_name, GroupName} = erlang:process_info(self(), registered_name),
          Pid ! {join, GroupName, Who},
    ...
    {broadcast, Who, Txt} ->
      ...
      loop(L);
    {broadcast_mp3, Who, Txt} ->
      broadcast(L, broadcast_mp3, Txt),
      loop(L);
    X ->
